...
// Each returns the link and whether it is the album itself, so the button can
// say which it is: "Apple Music" when it opens the record, "Search Apple
// Music" when it cannot promise to.
const Listen = {
  isAppleAlbumUrl,
  apple(album) {
    return isAppleAlbumUrl(album.apple_music_url)
      ? { href: album.apple_music_url, exact: true }
      : { href: `https://music.apple.com/search?term=${query(album)}`, exact: false };
  },
  spotify(album) {
    return { href: `https://open.spotify.com/search/${query(album)}`, exact: false };
  },
};
